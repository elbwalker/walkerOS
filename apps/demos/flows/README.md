...
```bash
# Build the server bundle with Dockerfile
npx walkeros bundle ./server.json --dockerfile
...
```bash
# Rebuild with BigQuery destination and Dockerfile
npx walkeros bundle ./server.json --dockerfile --no-cache
...
```bash
# Build the web bundle with Dockerfile
npx walkeros bundle ./web.json --dockerfile --no-cache
...
├── Dockerfile.collect  # Custom server Dockerfile (optional)
├── Dockerfile.serve    # Custom web Dockerfile (optional)
├── index.html          # Demo page
├── dist/               # Build output (generated by CLI)
│   ├── bundle.mjs      # Server bundle
│   ├── walker.js       # Web bundle
│   └── Dockerfile      # Auto-generated by --dockerfile
└── pentest/
    └── load-test.js    # k6 load test
```

> **Note:** The `--dockerfile` flag auto-generates the correct Dockerfile based
> on flow type. Use `--dockerfile Dockerfile.collect` to use a custom Dockerfile
> instead.
...
| Command                                           | Purpose                          |
| ------------------------------------------------- | -------------------------------- |
| `npx walkeros bundle ./server.json --dockerfile`  | Build server bundle + Dockerfile |
| `npx walkeros bundle ./web.json --dockerfile`     | Build web bundle + Dockerfile    |
| `npx walkeros bundle flow.json --dockerfile FILE` | Use custom Dockerfile            |
