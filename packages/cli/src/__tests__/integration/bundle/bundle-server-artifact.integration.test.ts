/**
 * Integration Test: server bundle artifact
 *
 * Builds a server flow config into the prebuilt artifact the runtime
 * (`runneros start`, from `@walkeros/runner`) consumes, and asserts its shape.
 * Starting that artifact is covered on the runtime side.
...
describe('Server bundle artifact', () => {
