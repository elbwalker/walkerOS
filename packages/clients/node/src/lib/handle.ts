...
export const handleCommand: NodeClient.HandleCommand = async (
  instance,
  action,
  data?,
  options?,
  ...
  let result:
    | Partial<NodeClient.PushResult | NodeDestination.PushResult>
    | undefined = {};

  switch (action) {
    case Const.Commands.Config:
      if (isObject(data))
        instance.config = getState(data as NodeClient.Config, instance).config;
      break;
...
export const handleEvent: NodeClient.HandleEvent = async (instance, event) => {
