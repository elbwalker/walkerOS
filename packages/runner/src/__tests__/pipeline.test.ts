...
describe('shouldStartHeartbeat', () => {
  it('is true when api is present', () => {
...
describe('runShutdown (extracted orchestrator)', () => {
  ...
  it('stops the tracePoller before the final beat', async () => {
    const calls: string[] = [];
    const tracePoller = {
      start: jest.fn(),
      stop: jest.fn(() => {
    ...
    await runShutdown(
      'SIGTERM',
      baseDeps({ tracePoller, heartbeat: handle, exit: jest.fn() }),
    );

    expect(calls).toEqual(['tracePoller.stop', 'sendOnce']);
  });

  it('drops /ready before the final beat and the collector drain', async () => {
    const calls: string[] = [];
    const { handle } = makeHeartbeat(() => {
      calls.push('sendOnce');
      return Promise.resolve();
    ...
    const healthServer: ShutdownDeps['healthServer'] = {
      setReady: jest.fn((ready: boolean) => {
        calls.push(`setReady(${ready})`);
      }),
      setFailed: jest.fn(),
      setDegraded: jest.fn(),
      close: jest.fn(() => Promise.resolve()),
    ...
    await runShutdown(
      'SIGTERM',
      baseDeps({ heartbeat: handle, collector, healthServer, exit: jest.fn() }),
    );
