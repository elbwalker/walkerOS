...
import type { ErrorRing } from './log-ring.js';
 ...
 *
 * Nothing else creates the cache dir: the runtime only ever loads a prebuilt
 * artifact and writes no bundle cache. On a fresh container the dir would be
 * missing, so every `appendFileSync` in
