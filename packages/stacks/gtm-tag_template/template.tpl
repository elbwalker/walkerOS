...

{
  ...
  "description": "Install and configure the walker.js",
  "containerContexts": [
    "WEB"
  ]
}
  ...
  },
  {
    ...
    "subParams": [
      ...
      },
      {
        ...
        "checkboxText": "Enable logging in preview mode",
        "simpleValueType": true,
    ...
    "subParams": [
      {
        "type": "LABEL",
        "name": "sessionNote",
        "displayName": "Session detection, user identification, and consent management are closely connected."
      },
      {
        "type": "GROUP",
        "name": "sessionGroupA",
    ...
    "subParams": [
      {
        ...
        "defaultValue": true,
        "help": "Enabling this option will eventually create the dataLayer and push every single event automatically."
      },
      {
        "type": "PARAM_TABLE",
        "name": "destinations",
...
function installation() {
  ...
  // Let's go
  callInWindow("Walkerjs.default", config);
...
setup: |-
  ...
  const mockWalkerjsVariable = {
    // misuse functions to use assertApi
    elb: mockElb,  // window.elb
    default: mockWalkerjs, // {{ walkerOS walker.js }}
  };
  const dataWalkerjsVariable = assign({
    walkerjsInstall: true,
    walkerjsInstallVia: 'variable',
    walkerjsInstallViaVariable: mockWalkerjsVariable
  }, data);

  // Utils
  const mockUtilsVariable = { foo: 'bar' };
  const dataUtilsVariable = assign({
    utilsInstall: true,
    utilsInstallVia: 'variable',
    utilsInstallViaVariable: mockUtilsVariable
  }, data);


___NOTES___

Created on 3/1/2024, 2:54:33 PM
