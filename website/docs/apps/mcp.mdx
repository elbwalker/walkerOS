...
| Package | Purpose |
|---------|---------|
| `@walkeros/mcp-flow` | Flow development — tools, reference resources, guided prompts, cloud API |
| `@walkeros/mcp-source-browser` | HTML tagging — generate, parse, and validate `data-elb` attributes |

## Quick start

Add servers to your Claude Desktop or Claude Code configuration:
...
|----------|---------|----------|---------|---------|
| `WALKEROS_TOKEN` | mcp-flow | No | — | Bearer token for cloud API and remote bundling |
| `WALKEROS_PROJECT_ID` | mcp-flow | No | — | Active project ID (`proj_...`) |
| `WALKEROS_APP_URL` | mcp-flow | No | `https://app.walkeros.io` | Base URL override |
...
## `@walkeros/mcp-flow` — flow development
...
<CodeSnippet
  code={`npm install @walkeros/mcp-flow`}
  language="bash"
/>
...
| `id` | string | No | Resource ID (flowId, projectId, or slug) |
...
unified in `@walkeros/mcp-flow`. Key changes:

| Old | New |
|-----|-----|
| `@walkeros/mcp-cli` | `@walkeros/mcp-flow` |
| `@walkeros/mcp-api` (17 tools) | `api` tool in `@walkeros/mcp-flow` |
