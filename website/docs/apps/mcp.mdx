...
| `flowId` | string | No | Flow ID. Required for: `deploy`. |
| `id` | string | No | Deployment slug (e.g. `dep_...`). Required for: `get`, `delete`. Must be a deployment slug, not a flow ID — resolve via `flow_manage` with action `get` if you only have a flow ID. |
| `projectId` | string | No | Project ID. Optional filter for `list`. |
