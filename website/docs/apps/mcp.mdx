...
The assistant calls `deploy({ action: "deploy", flowId: "cfg_abc123" })` and
...
| `@walkeros/mcp-api` (17 tools) | `auth` + `project_manage` + `flow_manage` + `deploy` tools |
| `api` (single mega-tool) | `auth`, `project_manage`, `flow_manage`, `deploy` (4 focused tools) |
