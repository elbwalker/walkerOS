...
- **Fetches secrets once at boot** and injects them into the environment before
  ...
  From then on every log line masks the fetched values (6 characters or
  longer) as `***`: the runtime's own lines, the flow's lines, the recent
  errors and log lines a heartbeat carries, and the `--json` error output.
