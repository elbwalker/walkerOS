...
Shows a hierarchical tree of the entities found in the DOM, with each entity's
properties resolved onto it the way the walker reads them:

- Generic (`data-elb-`) and scoped (`data-elb_`) values are shown on the
  entities that consume them, labeled by origin (`G` for generic, `S` for
  scoped; directly declared properties are unlabeled)
