...
The communication works completely asynchronously. The `elb` function pushes
...
type like `run` or `consent` changes.
...
as a value stored in a cookie for a longer period and the session can be used
