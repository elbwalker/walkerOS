...
And after the `run` command only `globals` and `tagging` can be updated.
...
The `on` command is used to add event listeners to the walker.js. If a type
(like `consent`) changes, the rules-matching function(s) will be called with the
parameters `instance`, `type`, and `options` (the current consent state).

```js
function marketingConsent(instance, options) {
  if (options.marketing) elb('walker user', readFromStorage());
}

//   command      type        rule       functions array
elb('walker on', 'consent', { marketing: [marketingConsent] });
```
