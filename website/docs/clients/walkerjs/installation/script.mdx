...
<Tabs>
  <TabItem value="script" label="Scripts" default>
    ...
    Using `data-default="true"` adds the `dataLayer` as the default destinations and
    starts the walker automatically. With `data-version` the current tagging version
    can be set. In both cases the `class="walkerjs"` is required to identify the
  ...
  <TabItem value="module" label="Module">
    ...
    <script type="module">
      ...
      const WalkerJs = (
        await import(
          'https://cdn.jsdelivr.net/npm/@elbwalker/client-web@latest/dist/index.mjs'
        )
      ).default;
