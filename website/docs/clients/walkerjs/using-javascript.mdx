...
Both <i>entity</i> and <i>action</i> are combined in one string separated by
space and required. The optional data object holds all properties of an entity.
The [trigger](https://docs.elbwalker.com/tagging/available-triggers) is optional
and is expected to be a string.
...
:::caution[Caution] When working directly with the elbLayer make sure to
initialize it properly by writing the following line before making use of the
push method: `elbLayer = elbLayer || [];` :::
