...
:::note Halt scopes
Transformer caches with `stop: true` in a source chain (`source.before`, `source.next`) or in the collector chain (`collector.next`) halt the event on HIT: no destination sees it. In a destination's `before` chain, a HIT skips the rest of that chain and the destination receives the cached event. Destination caches with `stop: true` halt only the per-destination delivery; sibling destinations still receive the event. To drop an event by rule rather than by cache, use a route `stop` (see [Dropping an event with `stop`](/docs/getting-started/flow/routing#dropping-an-event-with-stop)).
:::

The `dedup` step of
[`flow-complete.json`](https://github.com/elbwalker/walkerOS/blob/main/packages/cli/examples/flow-complete.json)
has no code of its own, only a cache. It runs in the server source's `next`
chain, so a hit with `stop: true` drops a resent event before the collector:

<FlowSlice feature="dedup-by-event-id" />

<FlowSlice feature="cache-stop" />

<FlowSlice feature="cache-namespace" />

<FlowSlice feature="cache-rules" />

<FlowSlice feature="cache-key" />

<FlowSlice feature="cache-ttl" />

<FlowExample feature="dedup-by-event-id" />
...
A thank-you page reload sends the same order again with a new event id, so the
`dedup` step above lets it through. Meta dedups on the order id instead, and the
rule's `match` limits the cache to `order complete`:

<FlowSlice feature="order-dedup-meta" />

<FlowSlice feature="cache-rule-match" />
...
A store can carry a cache of its own. In the complete example, reads from the
`customers` store are memoized for five minutes:

<FlowSlice feature="store-cache" />
