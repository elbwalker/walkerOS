...
<CodeSnippet
  code={`elb('walker on', type, subscription);`}
  language="javascript"
/>

`subscription` depends on `type` and can also be an array for multiple listeners
...
<CodeSnippet
  code={`elb('walker on', 'run', function (_, context) {
