...
Destinations define where your events are sent after processing. You can
configure destinations using either the traditional `createCollector`
destinations config or the more flexible `elb('walker destination', ...)`
approach:
...
For elegant configuration and to avoid config side-effects in tests, use the
`createSource` and `createDestination` utilities:
