...
Destinations define where your events are sent after processing. You can configure destinations using either the traditional `createCollector` destinations config or the more flexible `elb('walker destination', ...)` approach:
...
const { elb } = await createCollector(config);

// Add destinations using elb function
elb('walker destination', {
  type: 'analytics',
  push: async (event) => {
...
### Using createSource and createDestination Utilities

For elegant configuration and to avoid config side-effects in tests, use the `createSource` and `createDestination` utilities:

```typescript
import { createDestination, createSource } from '@walkeros/core';
...
// Create configured destination instance
elb('walker destination', createDestination(destinationGtag, {
  settings: {
    ga4: { measurementId: 'G-XXXXXXXXXX' },
    ads: { conversionId: 'AW-YYYYYY' }
  }
}));
```

**Benefits of createSource and createDestination:**
- **Fresh instances:** No config side-effects between collector instances
- **Type safety:** Full TypeScript support with proper generics  
- **Clean API:** Consistent pattern for sources and destinations
- **Single config:** Enables describing entire tracking setup in one place
- **Elegant configuration:** No more `{ code, config }` complexity
...
// Add console destination for debugging
elb('walker destination', {
  type: 'console',
  push: (event) => console.log('Event:', event),
});
...
// Add console destination for development debugging
elb('walker destination', {
  type: 'console',
