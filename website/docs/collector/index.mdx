...
`collector.status.failed` is the single counter for any event that did not
reach a destination cleanly. Two sites contribute:

* a destination's `push` threw or returned an error,
* an exception escaped the inner pipeline of `collector.push` or
  `collector.command` (mapping callback throws, transformer init throws,
  any uncaught error inside the boundary).
