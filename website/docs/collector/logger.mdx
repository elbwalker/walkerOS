...
across the collector, sources, and destinations. The logger supports four log
...
A collector endpoint is exposed to the open internet, so log level decides
whether ambient noise becomes an alert.
...
The collector already logs thrown pipeline failures at error, so sources do
not duplicate that. Nothing a scanner can trigger should reach `error`.


## In the complete example

The server collector of [`flow-complete.json`](https://github.com/elbwalker/walkerOS/blob/main/packages/cli/examples/flow-complete.json), the tested example flow, takes its log level
from a flow variable:

<FlowSlice feature="collector-logger" />
...
[Status — What `failed` counts](./#what-failed-counts) for details and
PII implications.
...
[Status — What `failed` counts](./#what-failed-counts).
