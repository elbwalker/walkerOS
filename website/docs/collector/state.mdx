...
- **Source:** `before` chain → state → `collector.push`. All entries run before the collector receives the event, in array order. State is skipped when the source uses a user `env.push` override, which receives the raw event with no pipeline.
