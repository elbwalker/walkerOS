...
**custom&nbsp;<Link to="/docs/collectors/server-collector/configuration">config</Link>** during
initialization or updated via **commands** using the `elb` function.

## elb

The `elb` function gets returned when creating a server collector. It pushes events
...
To&nbsp;<Link to="/docs/collectors/server-collector/configuration">configure</Link> the server
