...
title: Commands
sidebar_position: 3
description: Ways to configure the walkerOS server source using a function
---

import Link from '@docusaurus/Link';

A server collector can either be created with a
**custom&nbsp;<Link to="/docs/collectors/server/configuration">config</Link>**
during initialization or updated via **commands** using the `elb` function.

## elb

The `elb` function gets returned when creating a server collector. It pushes
events and commands to the instance and runs asynchronously by returning a
promise.
...
To&nbsp;<Link to="/docs/collectors/server/configuration">configure</Link> the
server collector either do it while creating an instance or use the
...
The `walker destination` command adds a destination to the server source.
