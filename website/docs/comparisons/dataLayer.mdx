...
E.g. you can add a [global attribute](/docs/sources/web/browser/tagging#globals) once
and automatically have it available in every event. walker.js also captures
[additional fields](/docs/sources/web/browser/) like event timings, consent-state,
event count or the tagging version automatically. The states of the events are
always up-to-date as a user interacts with certain components on the screen.
Using the dataLayer you'll have to update every single event on its own.
...
- **Use GTM destination:** If you'd like to use
  [walker.js](/docs/sources/web/browser/) as an event source while keeping GTM for
  other tasks, you can use the
  [Google Tag Manager destination](/docs/destinations/web/gtag/gtm)
