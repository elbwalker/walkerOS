...
In this post we will compare the traditional approach to
...
structures, queues, and routes events — including to GTM.
...
Svelte didn't dominate (or were even around yet). So the dataLayer design was: A
simple way to push events when pages loaded or user actions occurred.

It worked well for that era.

The problems begin in SPAs & Dynamic Apps. To make dataLayer work reliably in
SPAs or dynamic environments, developers must:

- Deal with asynchronously loading content
- Delay or wrap pushes to avoid timing issues
- Build custom buffering logic or re-loop the dataLayer for late-loaded vendors
  or consent updates
- Use additional trigger configurations like trigger groups or custom events

For sure these things aren't impossible — but they **require non-trivial effort
and discipline** to maintain at scale.

### walker.js

walker.js uses a structured event queue that immediately captures and holds
events until the system is ready to process them. It was built for SPA's and
modern websites by design. XXXXXXX

It automatically uses MutationObservers and event listeners to capture events
immediately — even before tools or components are fully loaded. It queues events
for late loading tools, and tracks context, consent, and state alongside each
event.
...
**✅ Advantage: Less hacky work for developers to handle state and race
conditions.**
...
GTM can support consent — but only through manual setup and coordination. To
...
It's doable — but fragile. One small timing issue, and a tag might fire before
...
**✅ Advantage: No accidentally shared data to vendors without the users'
...
@TODO relate to the problem

While you can technically push any data into `dataLayer`, it was specifically
designed for use within Google Tag Manager for Google products. For most
companies it quickly became the middleman to also send data to non-Google tools,
like Meta. And that's where things got fragile.

#### 🛠 Example: Sending a ViewContent event to Meta Pixel
...
@TODO A lot of clicking back and forth screen-to-screen.

⚠️ Your tracking logic — including **vendor mapping, event logic, consent
handling, firing rules** — is **spread across the GTM UI**, with limited
...
| ----------------------------- | ---------------------------- | ----------------------- |
| Logic → vendor mapping        | Spread across GTM UI         | In one place, your code |
| Controlled via source code    | ❌ No                        | ✅ Yes                  |
| Versioned in Git              | ❌ No                        | ✅ Yes                  |
| Sending correct vendor format | ⚠️ Manually mapping via UI   | ✅ Auto via config      |
| Consent-safe execution        | ⚠️ Manually add rules via UI | ✅ Built-in             |
| Adding new vendor             | Build new GTM setup          | Config-only change      |

**✅ Advantage: Tracking logic lives in **your** code — easy to document, test,
reuse, and change.**
