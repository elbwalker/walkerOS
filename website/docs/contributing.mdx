...
Some IDEs, like [Visual Studio Code](https://code.visualstudio.com/), have a
...
<CodeBox
  code={`# Make sure to install all dependencies first
npm i
...
<CodeBox
  code={`npx npm-check-updates -u && npm update && npm install
`}
  disabled
  language="bash"
  label="Code Example"
...
<CodeBox
  code={`$ npx changeset pre enter next
`}
  disabled
  language="bash"
  label="Code Example"
