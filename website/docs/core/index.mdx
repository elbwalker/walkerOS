...
sets nested values using dot notation, mutating the original object.

```js
setByPath(event, 'data.id', 'new-value');
// Updates event.data.id to 'new-value'
```
...
extracts values from events using <Link to="/docs/destinations/event-mapping">mapping
configurations</Link>.
