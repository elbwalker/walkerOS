...
<PackageLink
  env="server"
  github="packages/server/core"
  npm="@walkeros/server-core"
/>
...
`getHashServer(str: string, length?: number, options?: { algorithm?: 'sha256' | 'md5' }): Promise<string>`
generates hashes using Node.js crypto module. The algorithm defaults to
`sha256`; pass `{ algorithm: 'md5' }` for an MD5 digest.
