...
<CodeBox code={`import { getAttribute, sendWeb, sessionStart } from '@walkeros/web-core';`} disabled language="typescript" label="Code Example" showHeader={false} autoHeight={{ min: 100, max: 600 }} />
...
<CodeBox code={`type StorageType = 'localStorage' | 'sessionStorage' | 'cookie';`} disabled language="typescript" label="Code Example" showHeader={false} autoHeight={{ min: 100, max: 600 }} />
