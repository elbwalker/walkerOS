...
per element, not once for the whole array. Batched requests carry no
`traceparent` header: a batch can aggregate events from distinct traces, so
there is no single identity to stamp.
