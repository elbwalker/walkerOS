...
- The CLI emits a structured JSON line on stdout when the package's `setup()` returns a non-undefined value (useful for `jq` piping in CI).
...
- [walkeros setup CLI command](/docs/apps/cli#setup-command) for the operator-facing command, options, and exit codes.
- [Create your own source](/docs/sources/create-your-own) for source setup.
- [Stores](/docs/getting-started/stores) for store setup and lifecycle ordering.
