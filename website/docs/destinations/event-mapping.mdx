...
Event mapping is a core feature of walkerOS that allows you to transform events
before they reach destinations. The mapping system provides flexible
configuration options to handle different destination requirements while
maintaining a consistent event structure.

## Overview

The mapping system consists of two main functions:

- **`getMappingEvent`**: Returns the matching event mapping configuration for
  entity-action events
- **`getMappingValue`**: Processes values from events using flexible mapping
  configurations
...
For more detailed examples and use cases, see our
<Link to="/docs/getting-started/quickstart/custom-mapping-functions">Custom
Mapping Functions</Link> guide.
