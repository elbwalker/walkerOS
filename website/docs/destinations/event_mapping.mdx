...
process events individually. The `Mapping.Rule` is defined as
`{ [entity]: { [action]: Mapping.Rule }}`.
...
options. The `settings` option can be used to set up custom properties for the
...
| **settings**  | object   | Individual settings for a custom destination.                            |
...
A `ValueConfig` is like a recipe to produce a single value in the final payload.
This recipe can be broken down into three main parts:

- **Sources**: Where to get the raw value from.
- **Transformers**: How to shape or structure the value.
- **Guards**: Rules to decide if the value should be included.

### Sources

Where to get the raw value from.
...
How to shape or structure the value.
...
Rules to decide if the value should be included.
