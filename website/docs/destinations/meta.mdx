...
| fbclid          | string   | Facebook click ID parameter (will be formatted)                                                                                                  |
...
At least one `user_data` parameter is required. It can be set on
`custom.user_data` level, with the `data.map.user_data` config, or on the event
level with `mapping.entity.action.user_data`. All values will be merged
together.
