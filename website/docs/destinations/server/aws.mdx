...
The AWS SDK automatically reads credentials from environment variables. Configure the stream name using the `$env.NAME` syntax:
  ...
  "destinations": {
    "firehose": {
      ...
      "config": {
        "settings": {
          "firehose": {
            "streamName": "$env.AWS_FIREHOSE_STREAM",
