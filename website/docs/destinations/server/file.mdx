...
Use `$env.NAME` in your flow configuration to parameterise filenames at runtime:
...
<CodeSnippet
  code={`docker run -d \\
  -e LOG_FILE=/var/log/walkeros/events.jsonl \\
  -v /var/log/walkeros:/var/log/walkeros \\
...
:::tip Persist the volume
The container's filesystem is ephemeral. Mount a host path or named volume at the directory holding the log file so events survive container restarts.
:::
