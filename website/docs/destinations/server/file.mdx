...
<WhereThisFits
  pre={<>File is a <strong>server destination</strong> in the walkerOS flow:</>}
  flowMap={{
    destinations: { default: { label: 'Destination', text: 'Local file', highlight: true } },
  }}
  post={<>Receives events server-side from the collector, serialises each event, and appends a line to a local file. No network calls, no SDKs, just `fs.createWriteStream` with `flag: 'a'`.</>}
/>
