---
title: GCP (BigQuery, Pub/Sub)
description: "Stream events to Google BigQuery and publish to Pub/Sub topics"
...
The GCP destination package provides server-side integration with Google
Cloud. It ships two sub-destinations from one npm install:

- **BigQuery**, for streaming events into a managed data warehouse for
  analytics and machine learning workloads.
- **Pub/Sub**, for publishing events to a topic for fan-out, async
  processing, or cross-region delivery.

This page documents BigQuery first, Pub/Sub follows below.
...
Three modes (the same chain BigQuery uses):

1. **Application Default Credentials (ADC)**: nothing to configure beyond
   `projectId`. Works on GCP-native runtimes (Cloud Run, GKE) and locally
   with `gcloud auth application-default login`.
2. **Service account JSON**: pass credentials (object or JSON string) via
   `settings.credentials`.
3. **Pre-configured client**: pass an existing `PubSub` SDK instance as
   `settings.client`.
...
| ------------- | --------------- | -------------------------------------------- |
| `topic`       | `string`        | Override the default topic for this rule     |
| `orderingKey` | `Mapping.Value` | Per-rule ordering-key path                   |
| `attributes`  | `Mapping.Map`   | Per-rule attributes merged onto the defaults |

### Ordering

`orderingKey` is a `Mapping.Value` resolved per event (for example
`'user.id'`). Truthy values enable per-key ordering, the topic handle is
constructed with `messageOrdering: true`, and the resolved key flows on the
publish.

If a publish fails for an ordered key, Pub/Sub permanently halts subsequent
publishes for that key until `topic.resumePublishing(key)` is called. The
destination handles this automatically: on publish failure for an ordered
key it calls `resumePublishing` immediately and re-throws.

### Setup (one-time provisioning)

<CodeSnippet
  code={`walkeros setup destination.<id>`}
  language="bash"
/>

Provisions the topic if it does not already exist. Idempotent, safe to
re-run. Defaults:
...
The provisioning step needs `pubsub.topics.create` and `pubsub.topics.get`
on the project. The runtime service account needs `roles/pubsub.publisher`
on the topic itself.
...
continues. There is no auto-mutation. Migrations are an operator decision.

Subscription provisioning is owned by the Pub/Sub source, not the
destination.
...
The SDK automatically picks up `PUBSUB_EMULATOR_HOST`. To override
explicitly, set `settings.apiEndpoint`.

### Troubleshooting

- **NOT_FOUND on publish**: the topic does not exist. Run setup once.
...
- **Ordering stuck**: a previous publish for an ordering key failed. The
  destination calls `resumePublishing` automatically; if stalls persist,
  check publish-side error logs.
