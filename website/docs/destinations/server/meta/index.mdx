...
import { destinationMetaExamples } from '@walkerOS/server-destination-meta/examples';

<PackageLink github="packages/server/destinations/meta" npm="@walkerOS/server-destination-meta" />
...
- walkerOS&nbsp;<Link to="/docs/collectors/server-collector">server collector</Link> installed
...
<LiveCode
  ...
  output={`sendServer('https://graph.facebook.com/v22.0/${destinationMetaExamples.mapping.InitUserData.pixelId}/events?access_token=${destinationMetaExamples.mapping.InitUserData.accessToken}', {
    data: [
      {
