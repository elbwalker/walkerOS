...
<CodeSnippet
  ...
  "destinations": {
    "mparticle": {
      ...
      "config": {
        "settings": {
          "apiKey": "$env.MPARTICLE_API_KEY",
          "apiSecret": "$env.MPARTICLE_API_SECRET",
          "pod": "us1",
          "environment": "production"
        }
      }
...
<CodeSnippet
  code={`docker run -d \\
  -e MPARTICLE_API_KEY=your-api-key \\
  -e MPARTICLE_API_SECRET=your-api-secret \\
...
Never commit API keys or secrets to version control. Use environment variables or a secrets manager to inject credentials at runtime.
:::
