...
<WhereThisFits
  pre={<>PostHog is a <strong>server destination</strong> in the walkerOS flow:</>}
  flowMap={{
    destinations: { default: { label: 'Destination', text: 'PostHog', highlight: true } },
  }}
  post={<>Sends events server-side to PostHog's ingestion API, translating walkerOS events into `capture`, `identify`, and `groupIdentify` calls with full control over identity and group resolution.</>}
/>
