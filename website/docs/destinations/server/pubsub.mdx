...
export const data = {
  ...
  examples: gcpData.exportExamples?.destinationPubSub ?? {},
};
...
`walkeros push --simulate destination.pubsub` runs offline with the package's mock Pub/Sub client and prints the publish it would make; nothing reaches GCP. The step's `import` (`destinationPubSub`) selects the Pub/Sub examples and mock, not BigQuery's.
