...
Use the `$env.NAME` syntax to reference environment variables at runtime:
...
<CodeSnippet
  code={`docker run -d \\
  -e REDIS_URL=rediss://default:your-pass@host:6380 \\
  -e REDIS_STREAM=walkeros:events \\
...
Never commit Redis credentials to version control. Use environment variables or a secrets manager to inject credentials at runtime.
:::
