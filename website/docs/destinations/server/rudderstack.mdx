...
<WhereThisFits
  pre={<>RudderStack is a <strong>server destination</strong> in the walkerOS flow:</>}
  flowMap={{
    destinations: { default: { label: 'Destination', text: 'RudderStack', highlight: true } },
  }}
  post={<>Receives events server-side from the collector, resolves identity, and forwards them to RudderStack's data plane for routing to downstream destinations.</>}
/>
