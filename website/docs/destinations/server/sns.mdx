...
export const data = {
  ...
  examples: awsData.exportExamples?.destinationSNS ?? {},
};
...
`walkeros push --simulate destination.sns` runs offline with the package's mock SNS client and prints each `AWS.SNSClient.send` call with its command input; nothing reaches AWS. The step's `import` (`destinationSNS`) selects the SNS examples and mock, not Firehose's.
...
- [AWS SQS source](/docs/sources/server/sqs) for the consumer side. Standard pattern: SNS topic fans out to one or more SQS queues that walkerOS pulls.
- [Mapping configuration](/docs/mapping)
- [Flow configuration](/docs/getting-started/flow)
