...
Nested objects (`data`, `globals`, `consent`) are stored as JSON strings. `page_url` maps from `source.url`; `page_title` from `data.title`; `referrer_url` from `source.referrer`.
