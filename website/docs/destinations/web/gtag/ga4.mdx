...
rewrites those characters to underscores before sending, so a tagged
`creative-type` arrives as `data_creative_type` rather than going missing. Case
is left alone, since GA4 treats parameter names case-sensitively.
