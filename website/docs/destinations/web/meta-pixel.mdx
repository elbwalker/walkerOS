---
title: Meta Pixel
description: Facebook and Instagram advertising pixel integration
path: /docs/destinations/web/meta-pixel
sidebar_position: 2
package: '@walkeros/web-destination-meta'
---

import destinationMetaPixel from '@walkeros/web-destination-meta';
import data from '@walkeros/web-destination-meta/walkerOS.json';
import Settings from '@site/src/components/snippets/_settings.mdx';

import Mapping from '@site/src/components/snippets/_mapping.mdx';
export const destination = { ...destinationMetaPixel, examples: data.examples };

# Meta Pixel

<PackageLink
  env="web"
  github="packages/web/destinations/meta"
  npm="@walkeros/web-destination-meta"
/>

[Meta Pixel (formerly Facebook Pixel)](https://developers.facebook.com/docs/meta-pixel/)
tracks visitor activity and conversions for Facebook and Instagram advertising
campaigns. The destination automatically sends events to Meta Pixel via `fbq`
function. All you need to do is install the destination, and optionally map
events to Meta's format. walkerOS takes care of loading the script and the
function calls.

:::info Where This Fits
Meta Pixel is a **web destination** in the walkerOS flow:

<FlowMap
  sources={{ default: { highlight: false } }}
  collector={{ highlight: false }}
  destinations={{ default: { label: 'Destination', text: 'Meta Pixel', highlight: true } }}
/>

Loads Meta Pixel script dynamically, initializes the fbq function with your pixel ID, and tracks page views and custom conversions for Facebook and Instagram ads.
:::

## Installation

<CodeSnippet
  code={`npm install @walkeros/web-destination-meta`}
  language="bash"
/>

## Setup

<Tabs groupId="mode">
  <TabItem value="integrated" label="Integrated" default>

<CodeSnippet
  code={`import { startFlow } from '@walkeros/collector';
import { destinationMeta } from '@walkeros/web-destination-meta';

const { collector, elb } = await startFlow({
  destinations: {
    meta: {
      code: destinationMeta,
      config: {
        settings: {
          pixelId: '1234567890', // Your Meta Pixel ID
        },
        loadScript: true, // Load Meta Pixel script automatically
      },
    },
  },
});`}
  language="typescript"
/>

  </TabItem>
  <TabItem value="bundled" label="Bundled">

Add to your `flow.json` destinations:

<CodeSnippet
  code={`"destinations": {
  "meta": {
    "package": "@walkeros/web-destination-meta",
    "config": {
      "settings": {
        "pixelId": "1234567890"
      },
      "loadScript": true
    }
  }
}`}
  language="json"
/>

[See Bundled Mode setup →](/docs/getting-started/modes/bundled) | [CLI reference →](/docs/apps/cli)

  </TabItem>
</Tabs>

<Settings
  schema={data.schemas.settings}
  type="destination"
  configHref="/docs/destinations#configuration"
/>

#### Example

<DestinationInitDemo
  destination={destination}
  settings={{
    pixelId: '1234567890',
  }}
/>

<Mapping schema={data.schemas.mapping} />

## Examples

:::tip

The code data.examples are interactive. You can edit the code to generate your
individual event mapping.

:::

Learn more aboot
[Meta Pixel's Standard-Events.](https://developers.facebook.com/docs/meta-pixel/reference#standard-events)

### ViewContent

<DestinationDemo
  destination={destination}
  settings={{ pixelId: '1234567890' }}
  smallText={true}
  height={'512px'}
  event={data.examples.step.viewContent.in}
  mapping={data.examples.step.viewContent.mapping}
  generic={true}
/>

### AddToCart

<DestinationDemo
  destination={destination}
  settings={{ pixelId: '1234567890' }}
  smallText={true}
  height={'512px'}
  event={data.examples.step.addToCart.in}
  mapping={data.examples.step.addToCart.mapping}
  generic={true}
/>

### InitiateCheckout

<DestinationDemo
  destination={destination}
  settings={{ pixelId: '1234567890' }}
  smallText={true}
  height={'512px'}
  event={data.examples.step.initiateCheckout.in}
  mapping={data.examples.step.initiateCheckout.mapping}
  generic={true}
/>

### Purchase

<DestinationDemo
  destination={destination}
  settings={{ pixelId: '1234567890' }}
  smallText={true}
  height={'512px'}
  event={data.examples.step.purchase.in}
  mapping={data.examples.step.purchase.mapping}
  generic={true}
/>
