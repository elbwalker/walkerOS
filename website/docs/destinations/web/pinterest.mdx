...
import { examples, schemas } from '@walkeros/web-destination-pinterest/dev';

export const destination = { ...destinationPinterest, examples };
...
<DestinationDemo
  ...
  event={examples.step.pageViewRename.in}
  mapping={examples.step.pageViewRename.mapping}
  generic={true}
/>
...
<DestinationDemo
  destination={destination}
  settings={{ apiKey: '2612345678901' }}
  event={examples.step.productViewContent.in}
  mapping={examples.step.productViewContent.mapping}
  generic={true}
/>
...
<DestinationDemo
  destination={destination}
  settings={{ apiKey: '2612345678901' }}
  event={examples.step.orderCompleteCheckout.in}
  mapping={examples.step.orderCompleteCheckout.mapping}
  generic={true}
/>
...
data)` BEFORE the track call. The Pinterest JS tag auto-hashes `em` — pass raw
emails through, do NOT pre-hash.

<DestinationDemo
  destination={destination}
  settings={{ apiKey: '2612345678901' }}
  event={examples.step.userLoginLead.in}
  mapping={examples.step.userLoginLead.mapping}
  generic={true}
/>
...
<DestinationDemo
  destination={destination}
  settings={{ apiKey: '2612345678901' }}
  event={examples.step.identifyOnlySkip.in}
  mapping={examples.step.identifyOnlySkip.mapping}
  generic={true}
/>
...
Identity is diffed against the destination's runtime state — `pintrk('set',
...
that prevents a duplicate initial fire.
:::
