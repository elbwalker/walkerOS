...
import { examples, schemas } from '@walkeros/web-destination-posthog/dev';

export const destination = { ...destinationPostHog, examples };
...
Every walkerOS event becomes `posthog.capture(event.name, properties)` without
any mapping.
...
the new user ID, `$set` overwrites person properties, `$set_once` only sets
...
`posthog.setPersonProperties(...)` instead — useful for profile updates that
should not create a new identity.

<DestinationDemo
  destination={destination}
  settings={{ apiKey: 'phc_test' }}
  event={examples.step.profileUpdateSetPersonProperties.in}
  mapping={examples.step.profileUpdateSetPersonProperties.mapping}
  generic={true}
/>
...
<DestinationDemo
  destination={destination}
  settings={{ apiKey: 'phc_test' }}
  event={examples.step.orderCompleteWithInclude.in}
  mapping={examples.step.orderCompleteWithInclude.mapping}
  generic={true}
/>
