...
Context refers to the **state** or **environment** in which the event was
...
granted** by the user, which is crucial for subsequent data processing and
helpful to comply with privacy regulations.
...
```
walkerOS Event → [MAPPING] → Destination Format
```
