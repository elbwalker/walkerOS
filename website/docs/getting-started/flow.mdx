...
**Built-in Code Source:**

For custom source logic without external packages, use `code: true`:
...
<CodeSnippet
  ...
  "sources": {
    "manual": {
      ...
      "config": {
        "settings": {
          "init": "context.logger.info('Manual source initialized')",
          "push": "env.push({ name: 'manual event', data: input })"
        }
      }
    }
  }
...
**Built-in Code Transformer:**

For custom transformation logic without external packages, use `code: true`:
...
The push code has access to `event` and `context`. Return the modified event to continue, or `false` to drop the event from the chain.
...
You can chain inline code transformers with package-based transformers:
