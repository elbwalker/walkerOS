...
Without a contract, schema rules live inside each step's `validate:` block, duplicated across flows. As the number of flows, events, and destinations grows, they drift.

A contract pulls those rules into one named, inheritable shape:

- **Single source of truth.** Define `product add` requirements once. Every step that validates events references the same definition.
...
- **Decoupled from validation.** Contracts describe what events should look like; step-level `validate:` enforces them at runtime. Same contract, multiple enforcement points.
...
If your flow has a single throwaway event with no validation, you do not need a contract. Reach for one as soon as the same shape needs to hold across more than one flow or step.
...
- **[Validate](/docs/getting-started/flow/validate)**: step-level `validate:` primitive that consumes contracts
- **[Validator](/docs/transformers/validator)**: the underlying transformer package (deprecated in favor of step-level `validate:`)
