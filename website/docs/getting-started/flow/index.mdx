...
The examples on this page come from
[`flow-complete.json`](https://github.com/elbwalker/walkerOS/blob/main/packages/cli/examples/flow-complete.json),
a tested ecommerce setup with three flows: `web` in the browser, `server`
behind it, and `warehouse` reading the Pub/Sub topic into BigQuery. For a
minimal first flow, start with the [Quickstart](/docs/getting-started/quickstart/).

<FlowSlice feature="flows-multi" depth={2} />
...
<FlowSlice feature="platform" />

It sits in the flow's `config` next to the other build and runtime metadata:

<FlowSlice feature="flow-config" />
...
The server flow of the complete example uses the source, collector and
destination positions, shown step by step on the [Routing](./routing.mdx#where-transformers-run)
page.

All chain fields (`before` and `next`, including `collector.next`) accept the same route grammar: a transformer id, an array, or a route config (see [Conditional routing](./routing.mdx#conditional-routing)).

See [Transformers documentation](/docs/transformers/) for available transformers and custom transformer guide.

### Connection rules

Sources, transformers, the collector and destinations connect through chain fields (`before` and `next`). The connection table, chain resolution, route operators and dropping events with `stop` are on the [Routing](./routing.mdx) page.
...
One file holds several flows. The complete example splits by platform (`web`,
`server`, `warehouse`, see [Example](#example)); the same works per environment,
for example `development` and `production`.

Build specific flows using the CLI:

<CodeSnippet
  code={`# Build the web flow
walkeros bundle flow.json --flow web

# Build the server flow
walkeros bundle flow.json --flow server

# Build all flows (--all needs an output directory)
walkeros bundle flow.json --all -o dist/all`}
  language="bash"
/>

### Cross-flow references with `$flow`

Flows in the same file can reference each other's `config` block via `$flow.<name>.<path>`. The most common case is linking a web flow's destination to a server flow's URL. In the complete example the web `collect` destination sends to the server flow:

<FlowSlice feature="ref-flow" />

Cross-flow refs read values from another flow's `config` block (`platform`, `url`, `settings.*`), keeping web and server flows in sync without duplication. The bundler resolves `$flow.server.url` to the server flow's `config.url` at build time. Validate is lenient (warns on missing values), bundle is strict and fails loudly if the referenced value is empty, so production builds never ship with an unresolved URL.
...
<FlowSlice feature="root-variables" />

<FlowSlice feature="ref-var-whole" />

<FlowSlice feature="ref-var-inline" />
...
<FlowSlice feature="ref-env-default" />

<FlowSlice feature="ref-env" />
...
shown in parts on this page and across the docs. Its
[guide](https://github.com/elbwalker/walkerOS/blob/main/packages/cli/examples/flow-complete.md)
teaches it in 15 chapters. `@walkeros/cli/examples` exports the file and its
feature manifest.

Every flow declares its steps in named maps. The web flow's sources, and the
server flow's transformers and destinations:

<FlowSlice feature="flow-sources" depth={1} />

<FlowSlice feature="flow-transformers" depth={1} />

<FlowSlice feature="flow-destinations" depth={1} />
