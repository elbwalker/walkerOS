...
A `string` names a transformer (or a path, see below). A `Route[]` runs each entry in order as a pipeline. A `RouteConfig` is a disjoint union: it sets **exactly one** of `next` (continue to a single `Route`), `case` (first-match dispatch into a list of `Route`s), or neither (pure gate that only filters by `match`). `match` is optional everywhere; omit it to always match.

First-match dispatch is the `case` operator:
...
`case` entries are evaluated in order and the first matching `Route` wins. The trailing entry omits `match`, so it always matches and acts as the default branch. If no entry matches, the event passes through unchanged to the collector.
