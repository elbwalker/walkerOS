...
- **Server**: `-o ./dist/` (a directory: ESM `flow.mjs`, `package.json` and the traced `node_modules/`)
...
Start the artifact with the runtime:

```bash
npx --package=@walkeros/runner runneros start dist/flow.mjs
```

Or with Docker, mounting the whole directory:

```bash
docker run -p 8080:8080 -v "$PWD/dist:/app/flow:ro" walkeros/flow:<version>
```
...
| `runneros start` | Run a built server flow (the separate `@walkeros/runner` package) |
...
<CodeSnippet
  code={`# Build the artifact directory
...
Use a `walkeros/flow` version that ships `runneros`, ideally the CLI version you bundled with. See the [runtime](/docs/apps/runtime) for image tags, the canonical Dockerfile and every environment variable.
...
`destination.console` replaced by a mock, and prints the outcome, the matched
mapping rule and the recorded vendor calls; add `--json` to get the same result
as JSON.
...
- [**Runtime**](/docs/apps/runtime): Run built flows in production
