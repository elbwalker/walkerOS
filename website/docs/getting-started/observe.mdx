...
and the deployment does no observation work. The ingest token is the one
secret in this setup, which is why it lives in the deployment environment and
nowhere else.
...
part of the hosted app; `walkeros observe start <flowId>` starts one from the
terminal. Self-hosting? The `observers` escape hatch above streams the same
records to any function or endpoint you control. [What Cloud adds](https://app.walkeros.io)
:::
