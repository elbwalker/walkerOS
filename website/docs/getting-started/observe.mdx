...
2. **Preview**: open your page in the browser and watch events locally, see
   [integrated mode](/docs/getting-started/modes/integrated#see-your-event).
3. **Observe**: attach a running flow to an observation session and watch its
...
parsed, stored, or sent. When the session ends, the observer rejects the next
post and the runtime detaches and clears the stored credential on its own.
...
session decide what gets captured. On the server, `WALKEROS_OBSERVE_LEVEL` is
the same knob in environment form.
...
:::cloud
Observation sessions with a live feed and minted per-session credentials are
part of the hosted app; `walkeros observe start <flowId>` mints one from the
terminal and prints the web activation link (the `?elbObserve=` credential
already appended), the server environment trio, the session expiry, and the
received-record count, while sessions themselves live server-side and idle
out when nothing keeps them alive. Self-hosting? The `observers` escape hatch
