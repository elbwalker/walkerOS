...
This creates the artifact directory `dist/` (`flow.mjs`, `package.json` and the traced `node_modules/`).

:::note Environment variables are read at runtime
This is a server flow, so its `$env.NAME` references (`$env.GCP_PROJECT_ID`,
`$env.BQ_DATASET:walkerOS`, `$env.BQ_TABLE:events`, `$env.BQ_LOCATION:EU`) are
not baked into the bundle. The built flow reads them from the container's
environment when it starts, falling back to the default after the colon.
Only web flows bake `$env` values in, from `config.bundle.env`. You set the
values in the Dockerfile below.
:::
...
# Copy the whole artifact directory: flow.mjs needs its node_modules/
COPY dist/ /app/flow/

# Copy service account credentials
COPY sa-bigquery.json /app/sa-bigquery.json

# Read by the flow at runtime (see the note above)
ENV GOOGLE_APPLICATION_CREDENTIALS=/app/sa-bigquery.json
ENV GCP_PROJECT_ID=your-project-id
ENV BQ_DATASET=walkerOS
ENV BQ_TABLE=events
ENV BQ_LOCATION=EU
```

Replace `<version>` with the `walkeros/flow` version matching your CLI, see
[image tags](/docs/apps/runtime#image-tags). The image already starts
`/app/flow/flow.mjs` on port 8080, so no command or `BUNDLE` is needed. To
bundle inside Docker instead of on your machine, use the
[canonical two-stage Dockerfile](/docs/apps/runtime#canonical-dockerfile).
...
- Delete local files (`dist/`, `sa-bigquery.json`, `Dockerfile`)
