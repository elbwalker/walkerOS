...
* One configuration describes the entire tracking setup
* Full type safety, with configs linked to their packages
* No manual destination registration
* Code and configuration stay separate
...
Replace `data-elbaction` with `data-elbactions` to keep the same behavior for
the `click` and `submit` triggers:
...
`data-elbactions` only fires for the `click` and `submit` triggers. All other
triggers, such as `load`, `visible`, `impression`, `hover`, `scroll`, `wait`,
and `pulse`, fire only from `data-elbaction`, which applies to the nearest
entity. For those triggers the @elbwalker all-entities behavior is not
available.
...
Use `data-elbaction` for more precise tracking that fires fewer events:
...
**Use `data-elbactions` (all entities, `click` and `submit` only) when:**
