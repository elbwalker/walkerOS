...
<CodeSnippet
  code={`{
  // Source: Normalize incoming dataLayer events
  sources: {
    dataLayer: {
      config: {
        mapping: {
          add_to_cart: {  // GA4 event name maps directly
            name: 'product add',  // walkerOS entity-action format
            data: {
              map: {
                id: 'items.0.item_id',
  ...
  // Destination: Transform for Meta Pixel
  destinations: {
    meta: {
      config: {
        mapping: {
          product: {
            add: {
              name: 'AddToCart',  // Meta's event name
              data: {
                map: {
                  content_ids: 'data.id',
...
- `ignore: true`: the rule matched but **nothing happens**. No data
  transformation, no destination call, no side effects. Use for suppression.
...
The example above runs the destination's `identify()` side effect on
`user login` events but skips the destination's default forwarding call.
