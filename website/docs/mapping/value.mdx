...
array of those, which is a fallback chain: the first entry that resolves wins.
...
| [Set](#set)                     | `{ set: [Value, Value, …] }`              | Builds an array from several values, in order. Use when the destination expects a list.       |
...
`loop` and `set` each produce the value, so set exactly one. If you set several,
exactly one runs, in this order, and the rest never run: `loop`, `map`, `set`,
`key`, `fn`. `value` is the fallback when the producer returns nothing,
including a `loop` over something that is not a list. `condition`, `consent`
and `validate` filter the result and combine freely.
...
steps in one place. For an event field you can also rewrite it with a `policy`
entry and read it with `key`. Anything bigger belongs in a transformer.
...
Resolve every value and return them as an array, in order. A value that
resolves to nothing keeps its position as an empty entry.

```typescript
{ set: ['data.id', 'data.name'] } // ['P1', 'Shirt']
```

For a fallback chain, where the first value that resolves wins, use a plain
array instead: `['data.email', 'user.email', { value: 'anonymous' }]`.
