...
only one result is used: `loop`, `map` or `set` win over `key`, and `key` wins
over `fn`. `value` is the fallback when the producer returns nothing.
`condition`, `consent` and `validate` filter the result and combine freely.
