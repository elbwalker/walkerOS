...
3. **Push**: Push function type (typically `Elb.Fn`)
4. **Env**: Environment dependencies (extends `Source.BaseEnv`)
...
if (settings.captureClicks && document) {
  document.addEventListener('click', (e) => {
    elb('element click', { tag: (e.target as HTMLElement).tagName });
  });
}
```
...
- **Forward events**: Source's `push` typically just forwards to `elb`
