...
By default, `startFlow` uses the **first source's push method** as the returned `elb` function. This provides a cleaner API since sources typically offer enhanced functionality:
...
// elb is automatically browser.push (first source)
// collector.push is still available via collector.push
```

For multiple sources, you can explicitly mark which one is primary:

```typescript
const { elb } = await startFlow({
  sources: {
    browser: { code: sourceBrowser },
    dataLayer: { code: sourceDataLayer, primary: true }, // Override default
  },
});
...
Without any sources, `elb` defaults to `collector.push`.
