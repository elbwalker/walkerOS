...
Server sources require platform-specific handlers. For containerized deployments, see the [runtime](/docs/apps/runtime).
...
|---|---|
| `application/json` | Parsed as JSON. Malformed JSON is rejected with 400 before the pipeline runs. |
| `text/plain` | Read as text, then parsed as JSON when it is JSON (a `navigator.sendBeacon` payload, which browsers always send as `text/plain;charset=UTF-8`). Any other text, such as a GA4 gtag batch of newline-separated hit lines, stays the raw string. |
| anything else | Not read: `body` is `undefined`. |

Both parsers apply the 1mb limit. A raw text body reaches `ingest.body` as the
string and yields one empty event, so a `source.before` decoder such as
[transformer-ga4](/docs/transformers/ga4) can turn it into events. Without a
decoder that empty event is rejected as invalid input by the collector: 400 on
synchronous POST, counted as `rejected` after the ack in respond-first mode. It
never reaches a destination. This follows the shared
[request scope](/docs/sources/scope) contract, so fetch, GCP and AWS sources
behave the same way.

:::tip Health checks
Health check endpoints (`/health` and `/ready`) are provided by the [runtime](/docs/apps/runtime#health-checks), not by individual sources. This means health checks work regardless of which source type you use.
:::
...
| 400 | Rejected client input: unparseable `application/json` body (body echoes the parser message) or, on synchronous POST, an event the pipeline declared invalid (body echoes the validation message, e.g. `Event name is required`) |
...
Ingest is opt-in: without `config.ingest` the source forwards no request
metadata, so no client IP leaves your server by accident. The keys `ip` and
`userAgent` are the ones server destinations read by default (Meta, TikTok, Snapchat, Pinterest, Bing, Reddit, Criteo and X (Twitter) conversion APIs, Google Data Manager and Piwik PRO), so this
map is all they need to send the client IP and user agent. Each of those
destinations can turn either off with `ip: false` or `userAgent: false` in its
settings.
...
| Path | Description |
|------|-------------|
| `ip` | Express's `req.ip`. The source never enables Express's `trust proxy`, so this is always the socket peer address; behind a proxy that is the proxy, not the visitor, see [Client IP behind a proxy](#client-ip-behind-a-proxy) |
...
| `body` | Parsed JSON body, the raw string for non-JSON `text/plain`, or `undefined` |
...
The recommended pattern needs no source setting at all: let the infrastructure
that terminates the connection hand you the client address as a single-value
header it controls, and map that header with the ingest map you already have.
...
:::warning Trust only a header your own infrastructure controls
...
<CodeSnippet
  code={`"ip": {
  "fn": "$code:(req)=>req.headers['x-forwarded-for']?.split(',').at(-2)?.trim()"
}`}
  language="json"
/>

`.at(-2)` takes the entry one hop in from the right, which is the
proxy-verified client entry under the contract above; with fewer entries the IP
stays absent. Adjust the offset to your own proxy's documented behavior and
confirm it with a request from a known address before trusting it.
