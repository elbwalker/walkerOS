...
| 400 | Rejected client input: an unparseable `application/json` body (body echoes the parser message) or, on synchronous POST, an event the pipeline declared invalid (body echoes the validation message, e.g. `Event name is required`) |
