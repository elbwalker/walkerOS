...
`ip` is Express's `req.ip`, the socket peer address. Behind a load balancer, a
CDN or a serverless platform that is the last proxy in the chain, never the
...
| Fastly | `Fastly-Client-IP` | Set automatically |
...
header, either accept an absent IP (the pipeline treats a missing signal as
"measured less", never as a wrong value) or extract the correct hop explicitly
in the mapping, keeping the trust decision visible in your config:
...
The example trusts exactly one proxy (takes the entry one hop in from the
right). Adjust the offset to your topology and verify it with a request from a
known address.
