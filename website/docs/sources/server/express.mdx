...
POST bodies are read by content type. An `application/json` body must parse, or
the request is answered with 400. A `text/plain` body is parsed as JSON when it
is JSON (`navigator.sendBeacon` sends JSON as `text/plain`) and otherwise kept as
the raw string on `ingest.body`, so a `before` transformer can decode it, for
example batched gtag.js hits with the [GA4 transformer](/docs/transformers/ga4).
...
| `body` | Parsed request body, or the raw string for a `text/plain` body that is not JSON |
