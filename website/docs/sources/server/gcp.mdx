...
The event name field is `name`. Every other field of the body is forwarded to
the collector as-is, so `source` (carrying `release` and `trace` provenance)
rides through instead of being dropped.
