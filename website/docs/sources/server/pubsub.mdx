...
## Push webhook responses

| Status | Meaning |
|--------|---------|
| 200 | Message acknowledged |
| 400 | The message is permanently invalid, either it failed to decode or the pipeline declared the event invalid. A retry can never fix it, so the answer is a nack |
| 500 | The pipeline failed on an otherwise valid message, so Pub/Sub redelivers |

Because a permanently-invalid message is nacked rather than acknowledged, set a
dead-letter policy on the subscription. That policy, not the source, is the
