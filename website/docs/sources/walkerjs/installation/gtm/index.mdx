...
localStorage. There is a virtual rule to limit the age to 30 days for the device
ID and 30 minutes for a session ID, which updates with each new run.

### Destinations

Destinations are used to send events to other systems. **Use dataLayer** will
