...
walker.js handles all trigger initializations and race conditions, builds the
events with context, and distributes them based on consent states and mapping
definitions to any destinations.
...
accept optional parameters. A scroll trigger needs to know the percentage a user
scrolls down while a wait trigger wants to know the number of milliseconds until
the action gets triggered. Use brackets behind the trigger to pass that
information.
...
To use array types, add the `[]` suffix to a properties name, such as
...
Learn more about the walkerOS [event model](/docs/walkeros/event-model).
