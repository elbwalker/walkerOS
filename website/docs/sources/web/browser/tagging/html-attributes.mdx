...
walkerOS supports tagging inside **open** shadow DOM. Elements and actions
inside `mode: 'open'` shadow roots are automatically discovered and tracked.

**Closed shadow DOM** (`mode: 'closed'`) is not supported by design. The
browser prevents external access to closed shadow roots. Use `window.elb()` for
manual event pushing from closed web components.
