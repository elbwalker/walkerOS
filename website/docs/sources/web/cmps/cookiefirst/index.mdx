...
2. **cf_init event**: Listens for the `cf_init` window event and reads
   `window.CookieFirst.consent` each time it fires.

3. **cf_consent event**: Listens for the `cf_consent` window event and
   processes the consent state in its `detail`.
...
With the default mapping, `functional` is granted whenever CookieFirst reports
`necessary: true`, even if the visitor denied the `functional` category.
Categories missing from `categoryMap` are reported under their CookieFirst
name. The source reports nothing while `window.CookieFirst.consent` is `null`,
so `explicitOnly` has no effect for CookieFirst.
