...
When several category IDs map to the same group, the group is `true` if any of
them is active. With the default mapping, `C0001` (Strictly Necessary) keeps
`functional` granted even when the visitor denies `C0003` (Functional).
