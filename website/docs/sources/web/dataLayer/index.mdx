---
title: DataLayer
description: Integration with existing dataLayer implementations
path: /docs/sources/web/dataLayer
sidebar_position: 2
package: '@walkeros/web-source-datalayer'
---
import data from '@walkeros/web-source-datalayer/walkerOS.json';
import Configuration from '@site/src/components/snippets/_configuration.mdx';

import WhereThisFits from '@site/src/components/snippets/_where-this-fits.mdx';

<PackageLink github="packages/web/sources/datalayer" npm="@walkeros/web-source-datalayer" />

# DataLayer source

Integrates with existing Google Analytics 4 and GTM dataLayer implementations by intercepting `dataLayer.push()` calls.

<WhereThisFits
  pre={<>The DataLayer source is a <strong>web source</strong> in the walkerOS flow:</>}
  flowMap={{
    sources: { default: { label: 'Source', text: 'dataLayer', highlight: true } },
  }}
  post={<>It bridges existing GA4/GTM implementations to walkerOS destinations.</>}
/>

## Installation

<CodeSnippet
  code={`npm install @walkeros/web-source-datalayer`}
  language="bash"
/>

<Tabs groupId="mode">
  <TabItem value="integrated" label="Integrated" default>

<CodeSnippet
  code={`import { startFlow } from '@walkeros/collector';
import { sourceDataLayer } from '@walkeros/web-source-datalayer';

const { collector, elb } = await startFlow({
  sources: {
    dataLayer: {
      code: sourceDataLayer,
      config: {
        settings: {
          name: 'dataLayer', // Name of global dataLayer array
          prefix: 'dataLayer', // Event prefix for filtering
        },
      },
    },
  },
  destinations: {
    // Your destinations
  },
});`}
  language="typescript"
/>

  </TabItem>
  <TabItem value="bundled" label="Bundled">

Add to your `flow.json` sources:

<CodeSnippet
  code={`"sources": {
  "dataLayer": {
    "package": "@walkeros/web-source-datalayer",
    "config": {
      "settings": {
        "name": "dataLayer",
        "prefix": "dataLayer"
      }
    }
  }
}`}
  language="json"
/>

[See bundled mode setup](/docs/getting-started/modes/bundled) | [CLI reference](/docs/apps/cli)

  </TabItem>
</Tabs>

<Configuration type="source" data={data} />

## How it works

The dataLayer source intercepts `dataLayer.push()` calls and transforms them into walkerOS events:

1. **Intercepts** existing `dataLayer.push()` calls
2. **Filters** events based on prefix or custom filter function
3. **Transforms** dataLayer format to walkerOS event format
4. **Forwards** to collector for processing

<CodeSnippet
  code={`// Existing dataLayer code (unchanged)
window.dataLayer = window.dataLayer || [];
dataLayer.push({ event: 'purchase', value: 99.99 });

// Automatically captured and transformed by sourceDataLayer
// → Sent to walkerOS collector as standard event`}
  language="javascript"
/>

Every event carries the page context in `source`, like browser source events:
`{ type: 'dataLayer', platform: 'web', url, referrer }`, with `url` from
`window.location.href` and `referrer` from `document.referrer`. Both are read
when the event is emitted, so a push after an SPA navigation carries the new
URL, and entries queued before the source started get the URL at replay time.
Destinations that read `source.url` (such as Meta or Piwik PRO) use it for
dataLayer events without extra config.

## Custom filtering

Filter which dataLayer events get processed:

<CodeSnippet
  code={`const { elb } = await startFlow({
  sources: {
    dataLayer: {
      code: sourceDataLayer,
      config: {
        settings: {
          filter: (event) => {
            // Only process purchase and add_to_cart events
            if (typeof event === 'object' && event !== null) {
              const e = event as { event?: string };
              return e.event === 'purchase' || e.event === 'add_to_cart';
            }
            return false;
          },
        },
      },
    },
  },
});`}
  language="typescript"
/>

## Source-level mapping

The source emits walkerOS events whose `name` is `"<prefix> <gtag-action>"`. The collector splits `name` on the first space into `entity` and `action`, so the prefix becomes the entity and the gtag action becomes the action. Source-level mapping rules key on that pair:

<CodeSnippet
  code={`"sources": {
  "dataLayer": {
    "package": "@walkeros/web-source-datalayer",
    "config": {
      "settings": { "prefix": "dataLayer" },
      "mapping": {
        "dataLayer": {
          "add_to_cart": {
            "name": "product add",
            "data": {
              "map": {
                "id": "items.0.item_id",
                "name": "items.0.item_name",
                "price": "value",
                "currency": "currency",
                "quantity": "items.0.quantity"
              }
            }
          },
          "purchase": {
            "name": "order complete",
            "data": {
              "map": {
                "id": "transaction_id",
                "total": "value",
                "currency": "currency"
              }
            }
          }
        }
      }
    }
  }
}`}
  language="json"
/>

For the gtag commands `consent`, `config`, and `set`, the action equals the command name (any trailing token such as `update` or a measurement ID is dropped by the entity/action split). Branch on the dropped value via rule-level `condition` or `event.data` if you need to distinguish, for example, `consent default` from `consent update`.

If you set a custom `prefix` (for example `"gtag"`), use that string as the entity key: `mapping.gtag.add_to_cart`.

## Migration strategy

Use the dataLayer source for gradual migration from GA4/GTM:

### Phase 1: Add walkerOS alongside existing dataLayer

<CodeSnippet
  code={`// Existing code continues to work
dataLayer.push({ event: 'purchase', value: 99.99 });
// Now also captured by walkerOS`}
  language="typescript"
/>

### Phase 2: Map dataLayer events to walkerOS destinations

<CodeSnippet
  code={`destinations: {
  ga4: {
    code: destinationGtag,
    config: {
      mapping: {
        // Map dataLayer purchase to GA4 format
      }
    }
  }
}`}
  language="typescript"
/>

### Phase 3: Gradually replace dataLayer.push with elb()

<CodeSnippet
  code={`// Old
dataLayer.push({ event: 'purchase', value: 99.99 });
// New
elb('order complete', { total: 99.99 });`}
  language="typescript"
/>

## Next steps

* Review [event mapping](/docs/mapping) for transforming dataLayer events
* Check [GA4 destination](/docs/destinations/web/gtag/ga4) for Google Analytics integration
