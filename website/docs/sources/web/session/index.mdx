...
When a new session is detected, the source pushes a `session start` event. In storage mode it also sets `user.session` and `user.device` on the collector, so every event of the session carries them:
