...
The walkerOS tag template is part of the GTM stack. It allows an easy
integration of walker.js on a website. Once set up, a single `walkerOS` tag can
be used to load walker.js on all pages of a website via GTM. Besides it's own
[configuration](#options), [destinations](#destination) and
...
To prevent eventual conflicts with other scripts or existing variables, the
default names can be changed. Make sure to update the templates Permissions to
enable read/write access to the new names.

### Options

#### Tagging version

Specifying a tagging `version` helps managing and debugging a setup by knowing
which version was used during measurement. After changing the setup, increase
the version number.
...
Enabling the `preview` adds a destination that simply logs all events to the
console.
...
automatically send all events to the GTM `dataLayer` with `walkerjs: true` to
filter.
...
<Tabs>
  <TabItem value="add" label="Add destination" default>
    Both, **Code** and **Config** are **User-Defined Variables**. A destinations code can be some **custom** JavaScript or a **pre-build** version.
...
Write your own code instead of the `console.log` statements.
...
3. Paste the code in within the following code block:
...
On events are used to listen to specific **Trigger** to execute **Code**. Use a
...
<Tabs>
  <TabItem value="consent" label="On consent" default>
    ...
    The code has to be an `object` with the **consent state as key** and the **function as value**.
