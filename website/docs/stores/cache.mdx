...
Request-cache values are serialized by the cache layer before they reach the backing store, so any store backend (filesystem, S3, GCS, in-memory) can hold structured HTTP responses, not just the in-memory tier. Cached entries honor their TTL: an expired entry is treated as a miss and re-fetched from the backing.

Cached values must be JSON-safe plus `Buffer` (a `Date` becomes a string, an `Error` loses its fields), which is fine for HTTP responses, events, and push results.
