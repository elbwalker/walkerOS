...
To absorb the Sheets quota, wire the consuming transformer or destination with the core `Cache` config, backed by a fast in-memory store (`@walkeros/store-memory`):
