...
[`flow-complete.json`](https://github.com/elbwalker/walkerOS/blob/main/packages/cli/examples/flow-complete.json), the tested example flow, runs `bot` in the server's `collector.next`,
so every event carries a score, and its `eventFilter` step keeps likely bots
from the vendors:

<FlowSlice feature="bot-score" />

<FlowExample feature="bot-score" />
