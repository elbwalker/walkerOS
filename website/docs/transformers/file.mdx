...
|--------|------|---------|-------------|
| `prefix` | `string` | none | URL prefix to strip (e.g., `/static`) |
| `headers` | `Record<string, string>` | none | Default response headers (e.g., `Cache-Control`) |
| `mimeTypes` | `Record<string, string>` | none | Extension overrides (e.g., `{ ".wasm": "application/wasm" }`) |
