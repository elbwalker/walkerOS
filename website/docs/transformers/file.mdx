...
// Reference this same object in stores and in env: the collector swaps it
// for the initialized store instance at startup.
const files = {
  code: storeFsInit,
...
The `'$store.files'` string only works in a bundled `flow.json`, where the CLI resolves it. In `startFlow` it stays a string and the transformer fails.
