...
[`flow-complete.json`](https://github.com/elbwalker/walkerOS/blob/main/packages/cli/examples/flow-complete.json), the tested example flow, runs `fingerprint` first in the server's
`collector.next` and feeds the id to Piwik PRO as its visitor id:

<FlowSlice feature="fingerprint" />

<FlowExample feature="fingerprint" />
