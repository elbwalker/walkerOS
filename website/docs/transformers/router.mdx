...
3. On match, the router **branches** to the matched transformer chain, preserving the current event
4. The downstream parser can build a new event from `context.ingest` or work with the existing one
