---
title: Validate
description: 'Enforce JSON Schema contracts on events at runtime'
sidebar_position: 3
package: '@walkeros/transformer-validate'
---

import data from '@walkeros/transformer-validate/walkerOS.json';
import Configuration from '@site/src/components/snippets/_configuration.mdx';

# Validate

<PackageLink
  env="universal"
  github="packages/transformers/validate"
  npm="@walkeros/transformer-validate"
/>

Checks events against JSON Schema [contracts](/docs/getting-started/flow/contract) and records a verdict. In `pass` mode it annotates the event and continues so a downstream step can route on the result. In `strict` mode it drops invalid events by stopping the chain. The transformer runs on both web and server.

## Installation

<CodeSnippet
  code={`npm install @walkeros/transformer-validate`}
  language="bash"
/>

<Tabs groupId="mode">
  <TabItem value="integrated" label="Integrated" default>

<CodeSnippet
  code={`import { startFlow } from '@walkeros/collector';
import { transformerValidate } from '@walkeros/transformer-validate';

await startFlow({
  transformers: {
    validate: {
      code: transformerValidate,
      config: { settings: { contract: [contractWeb], mode: 'strict' } },
    },
  },
});`}
  language="typescript"
/>

  </TabItem>
  <TabItem value="bundled" label="Bundled">

<CodeSnippet
  code={`"transformers": {
  "validate": {
    "package": "@walkeros/transformer-validate",
    "config": { "settings": { "contract": ["$contract.web"], "mode": "strict" } },
    "next": "ga4"
  }
}`}
  language="json"
/>

  </TabItem>
</Tabs>

## How it works

The transformer reads the `{ ingest, event }` context and validates the canonical event. Each `contract` entry is a constraint: a `$contract.<name>` reference resolves to the entity-action schemas for the event, an inline JSON Schema applies to the whole event. All entries are AND-ed and every error is aggregated. Set `format: true` to additionally check the canonical partial event shape (field types and structure); required-field rules come from `contract`.

`mode` decides what happens to an invalid event:

- **`pass`** (default): write the verdict to the event and continue. A downstream destination or transformer routes on `event.source.valid`.
- **`strict`**: record the errors, then stop the chain so the event never reaches downstream steps.

The verdict and the error list are written to two different places. The boolean verdict goes onto the **event** (`output.isValid`, default `source.valid`) as analytics-grade data that travels with the event. The issue list goes onto the **ingest** (`output.errors`, default `validation`) as observer-visible diagnostics, never event data, so it survives even a strict-mode drop. Set either path to an empty string to skip that write.

The transformer never throws. A schema the engine cannot compile or apply (for example an unresolvable `$ref`) is reported as an issue with `level: "error"`, so `pass` still forwards the event and `strict` drops it like an invalid one. A field whose value is `undefined` counts as absent, as in JSON.

<Configuration type="transformer" data={data} />

## Reference a contract

The canonical place for event shapes is the top-level `contract` block. Reference it with `$contract.<name>`; the bundler resolves the reference to a concrete schema before deploy, so the runtime transformer only ever sees resolved schemas.

In [`flow-complete.json`](https://github.com/elbwalker/walkerOS/blob/main/packages/cli/examples/flow-complete.json)
the server flow's `validate` step checks every enriched event against the
`server` contract, which extends `default` with the visitor hash:

<FlowSlice feature="validate-transformer" />

<FlowExample feature="validate-transformer" />

It runs in `pass` mode, so an invalid event keeps flowing with
`source.valid: false` and the warehouse keeps the violation:

<FlowSlice feature="validate-pass" />

<FlowExample feature="validate-pass" />

The vendors only get valid events: their shared `eventFilter` step stops an
event whose verdict is `false`. Use `strict` instead when invalid events should
reach no destination at all.

<FlowSlice feature="event-filter-valid" />

<FlowExample feature="event-filter-valid" />

Add `format: true` to also check the canonical event shape, and `output` to
move the verdict (`isValid`, default `source.valid`) or the issue list
(`errors`, default `validation`).

## Filter unwanted events

There is no separate `ignore` or `filter` setting. To drop unwanted events (for example GTM lifecycle pings like `gtm.js` / `gtm.dom`), author an inline schema that rejects them and run `mode: "strict"`. A schema where `name` must NOT match `^gtm\.` fails those events while real events pass.

<CodeSnippet
  code={`"validate": {
  "package": "@walkeros/transformer-validate",
  "config": { "settings": {
    "contract": [{ "type": "object", "properties": { "name": { "not": { "pattern": "^gtm\\\\." } } } }],
    "mode": "strict"
  } }
}`}
  language="json"
/>

## Limitations

- The error list may contain an extra parent entry pointing at a `properties` wrapper per failure, a quirk of the underlying JSON Schema engine. The `isValid` verdict is unaffected: it is `false` exactly when there is at least one failure.
- Issues are emitted at `level: "error"`; there is no warn level.
- `flow_simulate` does not run a source's `policy` and `next` chain, so a green simulation does not prove a validate step placed there.

## Next steps

- **[Contract](/docs/getting-started/flow/contract)** - named, inheritable event schemas referenced via `$contract`
- **[Create your own](./create-your-own)** - build custom transformers
