...
`async function getHashNode(str: string, length?: number): Promise<string>`
generates a hash value from a string, usually used to anonymize or
fingerprinting. To create a string concatenate all values. Use `length` to limit
the output.
...
`async function getHashWeb(str: string, length?: number): Promise<string>`
generates a hash value from a string, usually used to anonymize or
fingerprinting. To create a string concatenate all values. Use `length` to limit
the output.
