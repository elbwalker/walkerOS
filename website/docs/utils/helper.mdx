...
Import the core utils from the `@walkerOS/utils` package.
...
Import the server utils from the `@walkerOS/utils/server` package.

```ts
import { getHashServer, sendServer } from '@walkerOS/utils/server';
```
...
Import the web utils from the `@walkerOS/utils/web` package.

```ts
import { getHashWeb, sendWeb } from '@walkerOS/utils/web';
```
