...
`onLog(message: unknown, verbose?: boolean)` is a function designed for consistent
and customizable logging across your application. It allows you to log messages
with a verbose option, aiding in better monitoring and debugging of your
code.
...
`requestToData(parameter: unknown): WalkerOS.AnyObject | undefined` converts a query string or URL-encoded string into a JavaScript object. It handles nested objects and arrays, and attempts to cast values to their appropriate types (numbers, booleans).
...
`requestToParameter(data: WalkerOS.AnyObject | WalkerOS.PropertyType): string` converts a JavaScript object or a primitive value into a URL-encoded query string. It handles nested objects and arrays, encoding all values appropriately for use in URLs.
