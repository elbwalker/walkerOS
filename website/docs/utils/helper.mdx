...
Import the core utils either directly from the `@walkerOS/utils` package or from
the collectors:

```ts
// Directly from the utils package
import { assign, anonymizeIP } from '@walkerOS/utils';

// Web environments
import { assign, anonymizeIP } from '@walkerOS/web-collector';

// Server environments
import { assign, anonymizeIP } from '@walkerOS/server-collector';
```
...
`onLog(message: unknown, verbose?: boolean)` is a function designed for
consistent and customizable logging across your application. It allows you to
log messages with a verbose option, aiding in better monitoring and debugging of
your code.
...
This function is for maintaining a standardized logging format is crucial.

### Request

#### requestToData

`requestToData(parameter: unknown): WalkerOS.AnyObject | undefined` converts a
query string or URL-encoded string into a JavaScript object. It handles nested
objects and arrays, and attempts to cast values to their appropriate types
(numbers, booleans).

```js
requestToData('a=1&b=true&c=foo%20bar&d%5B0%5D=x&d%5B1%5D=y&e%5Bkey%5D=value');
// Returns { a: 1, b: true, c: 'foo bar', d: ['x', 'y'], e: { key: 'value' } }
```

#### requestToParameter

`requestToParameter(data: WalkerOS.AnyObject | WalkerOS.PropertyType): string`
converts a JavaScript object or a primitive value into a URL-encoded query
string. It handles nested objects and arrays, encoding all values appropriately
for use in URLs.

```js
requestToParameter({ a: 1, b: true, c: 'foo bar' });
// Returns 'a=1&b=true&c=foo%20bar'
```
...
Import the server utils from the server collector:

```ts
import { getHashServer, sendServer } from '@walkerOS/server-collector';
```

### getHashServer

`async getHashServer(str: string, length?: number): Promise<string>` generates a
...
```js
await getHashServer('' + userAgent + language + domain + dayOfMonth, 16);
// Return a 16 character hash, like 47e0bdd10f04ef13
```

### sendServer

`async sendServer(url: string, data?: SendDataValue, options: SendServerOptions = {}): Promise<SendResponse>`
...
API endpoint from a server environment.
...
Import the web utils from the web collector:

```ts
import { getHashWeb, sendWeb } from '@walkerOS/web-collector';
```
