...
The `Mapping.Rules` stores all
individual&nbsp;<Link to="/docs/destinations/event_mapping#mapping-rule">`Mapping.Rule`</Link>
configurations for
...
The `settings` property is used by
each&nbsp;<Link to="/docs/destinations">destination</Link> individually.

## getMappingEvent

`getMappingEvent(event: WalkerOS.PartialEvent, mapping?: Mapping.Rules<unknown>): Mapping`
...
a `condition` will be checked to determine if the `Mapping.Rule` applies. It's
...
`getMappingValue(event: WalkerOS.DeepPartialEvent | unknown | undefined, mapping: Mapping.Value, options?: Mapping.Options): WalkerOS.Property | undefined`
...
individual&nbsp;<Link to="/docs/sources/html-tagging">tagging setups</Link> to
standard fields of the destination.
