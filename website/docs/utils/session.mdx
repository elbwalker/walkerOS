...
There are multiple ways to define and measure a session. Sessions
represent a period when a user actively engages with a website, often used
...
[on consent event](/docs/clients/walkerjs/commands#on-events) and won't start
...
`device` ids are also set via `elb('walker user', user);`.
