...
each request. The `localStorage` can be used to persist data for a longer period
and across all the browser's tabs. As a volatile memory for one specific task,
the `sessionStorage` can be a good choice in terms of short-term persistence,
...
storageWrite('elbUserId', 'us3r1d', 60, 'local');
```
