...
The Tagger is a chainable utility for generating `data-elb` attributes for HTML elements. It provides a clean, type-safe API to build structured tracking attributes that integrate seamlessly with walkerOS's [HTML tagging system](/docs/sources/html-tagging).
...
The `createTagger()` function creates a new tagger factory with optional configuration.
...
The tagger automatically escapes special characters that could break attribute parsing:
...
```typescript
import { createTagger, type TaggerConfig, type TaggerInstance } from '@walkerOS/core';
