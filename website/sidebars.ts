import type { SidebarsConfig } from '@docusaurus/plugin-content-docs';

/**
 * Creating a sidebar enables you to:
 - create an ordered group of docs
 - render a sidebar for each doc of that group
 - provide next/previous navigation

 The sidebars can be generated from the filesystem, or explicitly defined here.

 Create as many sidebars as you want.
 */
const sidebars: SidebarsConfig = {
  docsSidebar: [
    // Mixed approach: autogenerated with specific Sources override
    {
      type: 'category',
      label: 'Getting started',
      collapsed: false,
      items: [{ type: 'autogenerated', dirName: 'getting-started' }],
    },
    {
      type: 'category',
      label: 'Sources',
      link: {
        type: 'doc',
        id: 'sources/index',
      },
      items: [
        {
          type: 'category',
          label: 'Browser',
          items: [{ type: 'autogenerated', dirName: 'sources/web/browser' }],
        },
        'sources/web/dataLayer/index',
      ],
    },
    {
      type: 'category',
      label: 'Collector',
      link: {
        type: 'doc',
        id: 'collector/index',
      },
      items: [
        'collector/commands',
        'collector/configuration',
        'collector/testing',
      ],
    },
    {
      type: 'category',
      label: 'Destinations',
      link: {
        type: 'doc',
        id: 'destinations/index',
      },
      items: [
        // Web destinations - flattened
        'destinations/web/api',
        'destinations/web/meta-pixel',
        'destinations/web/piwikpro',
        'destinations/web/plausible',
        {
          type: 'category',
          label: 'Google Tag Manager',
          items: [
            'destinations/web/gtag/index',
            'destinations/web/gtag/ga4',
            'destinations/web/gtag/ads',
            'destinations/web/gtag/gtm',
          ],
        },
        // Server destinations - flattened
        'destinations/server/aws',
        'destinations/server/gcp',
        'destinations/server/meta-capi',
      ],
    },
    {
      type: 'category',
      label: 'Core',
      link: {
        type: 'doc',
        id: 'core/index',
      },
      items: ['core/server', 'core/web'],
    },
    {
      type: 'category',
      label: 'Guides',
      link: {
        type: 'doc',
        id: 'guides/index',
      },
      items: [
        {
          type: 'category',
          label: 'Consent',
          items: [{ type: 'autogenerated', dirName: 'guides/consent' }],
        },
        'guides/event-mapping',
        'guides/interactive-examples',
        'guides/user-stitching',
      ],
    },
    {
      type: 'category',
      label: 'Migrations',
      link: {
        type: 'doc',
        id: 'migrations/index',
      },
      items: ['migrations/elbwalker_to_walkeros'],
    },
    {
      type: 'category',
      label: 'Comparisons',
      items: ['comparisons/dataLayer'],
    },
    'contributing',
  ],
};

export default sidebars;
